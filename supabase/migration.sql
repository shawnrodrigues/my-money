...
create table if not exists public.profiles (
  ...
  email text not null, full_name text not null default 'Member', role public.user_role not null default 'member',
...
create table if not exists public.entries (
  ...
  status text not null check (status in ('spent', 'lent', 'repaid')),
  created_at timestamptz not null default now()
);
...
create table if not exists public.goals (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  title text not null, target_amount numeric(12,2) not null, saved_amount numeric(12,2) not null default 0,
...
create table if not exists public.notification_settings (
  ...
  reminder_days_before integer not null default 1, updated_at timestamptz not null default now()
);
alter table public.goals enable row level security;
alter table public.notification_settings enable row level security;

-- Confirm with: select count(*) from public.entries;
