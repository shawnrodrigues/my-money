...
function renderAuth() {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-password-toggle]').forEach((button) => {
    button.addEventListener('click', () => {
      const field = button.closest('.password-field')?.querySelector<HTMLInputElement>('input[name="password"]')
      if (!field) return
...
function sharePerson() { const message = personStatementMessage(); if (message) window.open(`https://wa.me/?text=${encodeURIComponent(`${message}\n\nPlease settle the balance when you can.`)}`, '_blank', 'noopener') }
async function sharePersonOnDiscord() { const message = personStatementMessage(); if (!message) return; try { await sendDiscordMessage(decorateStatementMessage(message)); showToast('Statement sent to Discord.') } catch (error) { showToast(errorText(error, 'Unable to send the statement to Discord.')) } }
