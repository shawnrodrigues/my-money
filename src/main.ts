...
async function sendDueReminders() {
  ...
  for (const entry of dueEntries) {
    const reminderKey = `my-money-discord-${entry.id}-${today()}`
    if (localStorage.getItem(reminderKey)) continue
    const content = `My Money reminder: ${entry.person || 'Someone'} owes ${money(entry.amount)} for ${entry.title}. Due ${longDate(entry.due_date!)}`
    const response = await fetch(settings.discord_webhook_url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content }) }).catch(() => null)
    if (response?.ok) localStorage.setItem(reminderKey, 'sent')
  }
}
...
function renderNotifications() { return `<section class="panel settings-panel"><div class="panel-head"><div><h2>Notifications</h2><p>Get a reminder before money is due back</p></div><span class="settings-badge">OPTIONAL</span></div><form id="notification-form" class="settings-form"><label class="toggle-row"><span><b>Repayment reminders</b><small>Send a Discord message when a due date is near.</small></span><input type="checkbox" name="enabled" ${settings.reminders_enabled ? 'checked' : ''}><i></i></label><label>Discord webhook URL<input name="webhook" type="url" value="${esc(settings.discord_webhook_url)}" placeholder="https://discord.com/api/webhooks/..."><small>Keep this webhook private. It is stored with your account settings.</small></label><label>Remind me <select name="days"><option value="0" ${settings.reminder_days_before === 0 ? 'selected' : ''}>On the due date</option><option value="1" ${settings.reminder_days_before === 1 ? 'selected' : ''}>1 day before</option><option value="3" ${settings.reminder_days_before === 3 ? 'selected' : ''}>3 days before</option><option value="7" ${settings.reminder_days_before === 7 ? 'selected' : ''}>1 week before</option></select></label><button class="primary-btn" type="submit">Save notification settings <span>→</span></button><p class="settings-note">Add a due date when lending money. My Money checks for reminders when you open the app; a scheduled Supabase function can later make this work even when the app is closed.</p></form></section>` }
