...
async function sendDueReminders() {
  ...
  const messages = [
    ...dueEntries.map((entry) => ({ key: `my-money-discord-${entry.id}-${today()}`, content: `My Money reminder: ${entry.person || 'Someone'} owes ${money(entry.amount)} for ${entry.title}. Due ${longDate(entry.due_date!)}` })),
    ...monthlyReminders.map(({ person, balance, key }) => ({ key, content: `My Money monthly reminder: ${person} still owes ${money(balance)}.` })),
  ]
  for (const message of messages) {
    if (localStorage.getItem(message.key)) continue
    const response = await fetch(settings.discord_webhook_url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: message.content }) }).catch(() => null)
    if (response?.ok) localStorage.setItem(message.key, 'sent')
  }
}
...
function renderNotifications() { return `<section class="panel settings-panel"><div class="panel-head"><div><h2>Notifications</h2><p>Get a reminder before money is due back</p></div><span class="settings-badge">MONTHLY DEFAULT</span></div><form id="notification-form" class="settings-form"><label class="toggle-row"><span><b>Repayment reminders</b><small>Send a monthly reminder while someone still owes you, plus due-date reminders.</small></span><input type="checkbox" name="enabled" ${settings.reminders_enabled ? 'checked' : ''}><i></i></label><label>Discord webhook URL<input name="webhook" type="url" value="${esc(settings.discord_webhook_url)}" placeholder="https://discord.com/api/webhooks/..."><small>Keep this webhook private. It is stored with your account settings.</small></label><label>Due-date reminder <select name="days"><option value="0" ${settings.reminder_days_before === 0 ? 'selected' : ''}>On the due date</option><option value="1" ${settings.reminder_days_before === 1 ? 'selected' : ''}>1 day before</option><option value="3" ${settings.reminder_days_before === 3 ? 'selected' : ''}>3 days before</option><option value="7" ${settings.reminder_days_before === 7 ? 'selected' : ''}>1 week before</option></select></label><button class="primary-btn" type="submit">Save notification settings <span>→</span></button><p class="settings-note">Add a person and record repayments to receive one monthly reminder for each outstanding balance. My Money checks when you open the app; a scheduled Supabase function is needed when it is closed.</p></form></section>` }
...
function bindShell() {
  ...
  document.querySelector<HTMLFormElement>('#person-form')?.addEventListener('submit', async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget as HTMLFormElement); const name = String(form.get('name')).trim(); entryMessage = ''; if (people.some((person) => person.name.toLowerCase() === name.toLowerCase())) { entryMessage = 'That person is already in your directory.'; render(); document.querySelector<HTMLDivElement>('#person-modal')?.classList.remove('hidden'); return } try { await savePerson({ name, email: String(form.get('email')).trim(), phone: String(form.get('phone')).trim() }); document.querySelector<HTMLDivElement>('#person-modal')?.classList.add('hidden'); render() } catch (error) { entryMessage = errorText(error, 'Unable to save this person.'); render(); document.querySelector<HTMLDivElement>('#person-modal')?.classList.remove('hidden') } })
...
function bindModalForms() {
  ...
  entryForm?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      await saveEntry({ title: String(form.get('title')).trim(), amount: Number(form.get('amount')), date: String(form.get('date')), due_date: dueDate || null, category: String(form.get('category')), person: String(form.get('person')).trim(), note: String(form.get('note')).trim(), status: form.get('status') as Status })
